<?php $this->layout('layouts/layout', ['title' => "Главная", 'state' => json_encode($state)]); ?>
            ...
            <div id="c_1" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="<?=$user['username']?>">
                <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                    <div class="d-flex flex-row align-items-center">
                                <span class="status <?php if($state['id'] == $user['id']) echo 'status-success'; ?> mr-3">
                        ...
                        <div class="info-card-text flex-1">
                            <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false">
                                <?=$user['username']?>
                                <?php if($state && $state['id'] == $user['id'] || $state && $state['role'] == 1) { ?>
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="/edit_profile<?php if($state && $state['role'] == 1) echo "/{$user['id']}"?>">
                                    <i class="fa fa-edit"></i>
                                    Редактировать</a>
                                <a class="dropdown-item" href="/security<?php if($state && $state['role'] == 1) echo "/{$user['id']}"?>">
                                    <i class="fa fa-lock"></i>
                                    Безопасность</a>
                                <a class="dropdown-item" href="status.html">
                                    <i class="fa fa-sun"></i>
                                    Установить статус</a>
                                <a class="dropdown-item" href="/media<?php if($state && $state['role'] == 1) echo "/{$user['id']}"?>">
